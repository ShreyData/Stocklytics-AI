...
@router.get("/dashboard/live", status_code=200)
async def get_live_dashboard_summary(user: AuthenticatedUser = Depends(require_auth)):
    response_data = await analytics_service.get_live_dashboard_summary(user.store_id)
    return success_response(response_data, status_code=200)

@router.get("/sales-trends/live", status_code=200)
async def get_live_sales_trends(
    ...
    response_data = await analytics_service.get_live_sales_trends(
...
@router.get("/product-performance/live", status_code=200)
async def get_live_product_performance(user: AuthenticatedUser = Depends(require_auth)):
    response_data = await analytics_service.get_live_product_performance(user.store_id)
    return success_response(response_data, status_code=200)
...
@router.get("/customer-insights/live", status_code=200)
async def get_live_customer_insights(user: AuthenticatedUser = Depends(require_auth)):
    response_data = await analytics_service.get_live_customer_insights(user.store_id)
    return success_response(response_data, status_code=200)
